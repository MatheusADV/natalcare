...
class resultados(View):
    ...
    def post(self, request):
        ...
        if not respe and not resp and not rese and not resi:
            mens = "Clínica não encontrada"
        return render(request, 'site/pesquisa/pesq.html', {"espec": rese, "plano": resp, "inpu": resi, "esp": especialidades.objects.all(), "plan": plano.objects.all(), "respe": respe, "mens": mens})
...
class clinedit(View):
    def get(self, request):
        ...
        return render(request, 'site/perfil/clined.html', {"esp": clin_has_esp, "plan": clin_has_plan, "cdhe": clin_dont_has_esp, "cdhp": clin_dont_has_plan})
...
class chatusu(View):
    def get(self, request):
        ...
        return render(request, 'site/chat/chatusu.html', {"conv_iniciada": conv_iniciada})

class prochatusu(View):
    def get(self, request, id):
        clinid = clinica.objects.get(id=id)
